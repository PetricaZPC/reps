...
import { AssistantContext, SmartResponse, WorkoutLogEntry, askAssistant } from "../src/useGemini";
...
function NutritionCard({
  ...
  return (
    <View style={nc.card}>
      ...
      <View style={nc.pills}>
        {[
          { label: "kcal", value: data.calories, color: C.warning,  bg: C.warningLight },
          { label: "prot", value: `${data.protein}g`, color: C.accent, bg: C.accentLight },
          { label: "carbs", value: `${data.carbs}g`, color: "#A855F7", bg: "#F3E8FF" },
          { label: "fat",  value: `${data.fat}g`,  color: "#64748B", bg: "#F1F5F9" },
        ].map((m) => (
      ...
      {/* Progress bars */}
      <View style={nc.bars}>
        <ProgressRow label="Calorii azi" pct={calPct} left={`${calLeft} kcal rămase`} color={C.warning} />
        <ProgressRow label="Proteine azi" pct={protPct} left={`${protLeft}g rămase`} color={C.accent} />
      </View>
  ...
  return (
    <View style={nc.barWrap}>
      ...
      <View style={nc.barTrack}>
        <View style={[nc.barFill, { width: `${pct}%` as any, backgroundColor: color }]} />
      </View>
...
const nc = StyleSheet.create({
  ...
  barTrack: { height: 6, borderRadius: 3, backgroundColor: C.bg, overflow: "hidden" },
...
export default function Assistant() {
  ...
  return (
    ...
    >
      ...
      <View style={[s.blob, { bottom: 120, left: -80, width: 220, height: 220, backgroundColor: C.blob2 }]} />
      ...
      {/* Input */}
      <View style={[s.inputArea, { paddingBottom: Math.max(insets.bottom + 8, 16) }]}>
        <TextInput
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.85}
        >
          <Ionicons name="send" size={17} color={canSend ? "#fff" : C.textLight} />
        </TouchableOpacity>
      </View>
    </KeyboardAvoidingView>
  );
...
const s = StyleSheet.create({
  ...
  header: {
    ...
    backgroundColor: C.glass, borderBottomWidth: 1, borderBottomColor: C.border,
  },
  backBtn: {
  ...
  targetPill: { backgroundColor: C.accentLight, borderRadius: 20, paddingHorizontal: 10, paddingVertical: 5 },
  ...
  messageList: { paddingHorizontal: 16, paddingTop: 14, paddingBottom: 8, gap: 10 },

  userRow: { flexDirection: "row", justifyContent: "flex-end" },
  userBubble: {
    backgroundColor: C.userBubble, borderRadius: 20, borderBottomRightRadius: 5,
  ...
  aiBubble: {
    backgroundColor: C.surface, borderRadius: 20, borderBottomLeftRadius: 5,
  ...
  inputArea: {
    ...
    backgroundColor: C.glass, borderTopWidth: 1, borderTopColor: C.border,
  },
  input: {
    flex: 1, backgroundColor: C.surface, borderRadius: 18,
    borderWidth: 1.5, borderColor: C.border,
    paddingHorizontal: 16, paddingVertical: 12,
    fontSize: 15, color: C.text, maxHeight: 120, minHeight: 48,
  },
