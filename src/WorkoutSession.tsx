...
const fallbackExercise = (key: string): ExerciseConfig => ({
  ...
  minAngle: 80, maxAngle: 150,
  side: "both", countOn: "up", type: "reps",
...
export default function WorkoutSession({ preset, customRestTime, onFinish, onExit }: Props) {
  ...
  const hudTopOffset = isLandscape ? 8 : insets.top + 10;
  const hudLeftOffset = isLandscape ? insets.left + 12 : 12;
  const lightTopPadding = isLandscape ? 8 : insets.top + 16;
  ...
  // HUD compact pentru landscape — sus stanga, nu se intinde
  const ExerciseHUD = () => (
    <View style={[
      s.hudWrapper,
      isLandscape ? s.hudWrapperLandscape : s.hudWrapperPortrait,
      { top: hudTopOffset, left: hudLeftOffset }
    ]}>
  ...
  if (phase === "exercise_intro") {
    return (
      <View style={s.root}>
        ...
        <ProgressBar />
        {isLandscape ? (
          <View style={[s.landscapeContainer, { paddingTop: lightTopPadding, paddingLeft: insets.left + 20 }]}>
            <View style={s.landscapeLeft}>
              ...
              <Text style={[s.introName, { fontSize: 20 }]}>{currentExercise.name}</Text>
              <Text style={[s.introDesc, { fontSize: 12 }]}>{currentExercise.description}</Text>
            </View>
            <View style={s.landscapeRight}>
              <View style={s.targetChip}>
                ...
                <Text style={s.targetText}>
                  {totalSets} sets × {target} {currentExercise.type === "timed" ? "sec" : "reps"}
                </Text>
              </View>
              <View style={s.cameraHint}>
                <Ionicons name="phone-portrait-outline" size={13} color={C.warning} />
                <Text style={s.cameraHintText}>{currentExercise.cameraPosition}</Text>
              </View>
              <TouchableOpacity style={[s.startBtn, { paddingVertical: 12, paddingHorizontal: 28 }]} onPress={() => setPhase("exercise")} activeOpacity={0.88}>
                <Ionicons name="play" size={16} color="#fff" />
                <Text style={[s.startBtnText, { fontSize: 14 }]}>Let's go</Text>
              </TouchableOpacity>
              <TouchableOpacity style={s.dangerBtn} onPress={onExit}>
                <Text style={s.dangerBtnText}>End workout</Text>
              </TouchableOpacity>
            </View>
          </View>
        ) : (
          ...
          <View style={[s.landscapeContainer, { paddingTop: lightTopPadding, paddingLeft: insets.left + 20 }]}>
            <View style={s.landscapeLeft}>
              <Text style={s.restLabel}>
                {isBetweenEx ? "Rest between exercises" : "Rest between sets"}
              </Text>
              <View style={s.timerCard}>
                <Text style={[s.timerValue, { fontSize: 52 }]}>{fmt(restCountdown)}</Text>
                <Text style={s.timerSub}>seconds remaining</Text>
              </View>
            </View>
            <View style={s.landscapeRight}>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#000" }}>
      ...
      {/* HUD compact — ramane mic indiferent de orientare */}
      <ExerciseHUD />
...
const s = StyleSheet.create({
  ...
  centered: { flex: 1, alignItems: "center", justifyContent: "center", paddingHorizontal: 28, gap: 12 },
  ...
  landscapeLeft: { flex: 1, alignItems: "center", justifyContent: "center", gap: 8, maxWidth: 260 },
  landscapeRight: { flex: 1, alignItems: "center", justifyContent: "center", gap: 8, maxWidth: 260 },

  stepChip: { backgroundColor: C.accentLight, borderRadius: 20, paddingHorizontal: 14, paddingVertical: 5 },
  stepText: { fontSize: 12, fontWeight: "700", color: C.accent },
  exerciseIconWrap: { width: 64, height: 64, borderRadius: 20, backgroundColor: C.accentLight, alignItems: "center", justifyContent: "center", marginBottom: 4 },
  introName: { fontSize: 26, fontWeight: "700", color: C.text, textAlign: "center", letterSpacing: -0.5 },
  introDesc: { fontSize: 14, color: C.textMuted, textAlign: "center", lineHeight: 20 },
  targetChip: { flexDirection: "row", alignItems: "center", gap: 6, backgroundColor: C.accentLight, borderRadius: 10, paddingHorizontal: 14, paddingVertical: 9 },
  targetText: { fontSize: 13, color: C.accent, fontWeight: "600" },
  cameraHint: { flexDirection: "row", alignItems: "center", gap: 7, backgroundColor: C.warningLight, borderRadius: 10, paddingHorizontal: 14, paddingVertical: 9 },
  cameraHintText: { fontSize: 13, color: C.warning, fontWeight: "500" },
  startBtn: { flexDirection: "row", alignItems: "center", gap: 10, backgroundColor: C.accent, borderRadius: 16, paddingVertical: 16, paddingHorizontal: 48, marginTop: 8, shadowColor: C.accent, shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.28, shadowRadius: 14, elevation: 6 },
  ...
  timerCard: { backgroundColor: C.glass, borderRadius: 24, borderWidth: 1, borderColor: C.glassBorder, paddingHorizontal: 48, paddingVertical: 24, alignItems: "center", shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 16, elevation: 2 },
  timerValue: { fontSize: 72, fontWeight: "700", color: C.warning, letterSpacing: -2 },
  timerSub: { fontSize: 13, color: C.textMuted, marginTop: 4 },
  nextCard: { backgroundColor: C.glass, borderRadius: 20, borderWidth: 1, borderColor: C.glassBorder, padding: 20, alignItems: "center", width: "100%", shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 16, elevation: 2 },
  nextLabel: { fontSize: 10, fontWeight: "700", color: C.accent, letterSpacing: 1.4, textTransform: "uppercase", marginBottom: 8 },
  ...
  skipBtn: { flexDirection: "row", alignItems: "center", gap: 8, borderRadius: 14, borderWidth: 1.5, borderColor: C.accent, paddingHorizontal: 24, paddingVertical: 13 },
  skipBtnText: { color: C.accent, fontSize: 15, fontWeight: "700" },

  cameraProgress: { position: "absolute", top: 0, left: 0, right: 0, height: 3, backgroundColor: "rgba(0,0,0,0.3)", zIndex: 50 },
  cameraProgressFill: { height: 3, backgroundColor: C.accent },

  // HUD compact — nu se intinde, ramane mic
  hudWrapper: { position: "absolute", flexDirection: "row", gap: 8, zIndex: 40 },
  hudWrapperPortrait: { right: 12 },
  hudWrapperLandscape: { right: 12 },

  hudInfo: { backgroundColor: "rgba(247,248,250,0.88)", borderRadius: 12, padding: 10, borderWidth: 1, borderColor: "rgba(255,255,255,0.9)", maxWidth: 180 },
  hudStep: { fontSize: 10, color: C.textMuted },
  hudName: { fontSize: 13, fontWeight: "700", color: C.text, marginTop: 1 },
  hudCounter: { backgroundColor: "rgba(247,248,250,0.88)", borderRadius: 12, padding: 10, alignItems: "center", minWidth: 70, borderWidth: 1, borderColor: "rgba(255,255,255,0.9)" },
  hudCountValue: { fontSize: 22, fontWeight: "700", color: C.warning, letterSpacing: -0.5 },
  hudCountTarget: { fontSize: 10, color: C.textMuted, marginTop: 1 },

  hudBottom: { position: "absolute", left: 16, right: 16, flexDirection: "row", gap: 10, zIndex: 40 },
  hudEndBtn: { flexDirection: "row", alignItems: "center", gap: 6, backgroundColor: "rgba(239,68,68,0.85)", borderRadius: 12, paddingHorizontal: 18, paddingVertical: 13 },
  hudEndBtnText: { color: "#fff", fontWeight: "700", fontSize: 14 },
  hudSkipBtn: { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, backgroundColor: "rgba(247,248,250,0.88)", borderRadius: 12, paddingVertical: 13, borderWidth: 1, borderColor: "rgba(255,255,255,0.9)" },
